...
var BGMusic, JumpSound, DragonRoar, DragonFire,EndLV1,DemonRoaring;
...
var BossMusic, demonRoar,levelTwoMusic; // sounds
...
function preload() {
 ...
 // noLoop();
  ...
  // Image for effects
 DemonRoaring = loadImage("Images 2/Attack 2.png");
  ...
  BossMusic = loadSound("Sounds/bossMusic.mp3");
  demonRoar = loadSound("Sounds/Demon Roar.mp3");
  
}

function setup() {
  createCanvas(windowWidth,windowHeight); ///// fix this and make the background adjust and stuff
...
// Gravity for Monskey
  Monskey.velocityY = Monskey.velocityY + 1;
   ...
   if(Monskey.velocityX === -10 && Monskey.velocityY === 0) {
    Demon.x = 17400;
    Demon.x = Monskey.x;
    Demon.velocityY = 5;
    }

   if(Monskey.velocityX === 8 && Monskey.y < 380) {
   Demon.x = 17400;
   Demon.x = Monskey.x;
   Demon.velocityY = 5;
   } else if(Monskey.y > 380) {
     Demon.velocityY = -5;//////////////////////////fix this issue 
     Demon.x = Monskey.x; 
   }
   Demon.debug = true;
   Demon.setCollider("rectangle",0,0,50,50);

   if(Demon.isTouching(Monskey)) {
     Demon.velocityY = 0;
     Monskey.velocityX = 0;
     Monskey.velocityY = 0;
   }
...
if(keyDown(DOWN_ARROW)) {
 ...
 if(keyCode === 32 && gameState === "level1Over") {
  ...
  setTimeout(function destroyChat4() {chatGroup2.destroyEach();},13000);
  setTimeout(goToCave,16000);
